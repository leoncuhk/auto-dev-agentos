...
### 2. 基线 (Baseline)
当前的最基础的双均线交叉策略（10日线穿20日线出金叉），夏普比率为 `0.84`。
...
*   如果你的某次修改（实验）导致回测脚本抛出异常，或 Sharpe Ratio 跌出 0.84 以下，**你必须把代码回滚 (Revert)**，并将这次修改的思路以及导致失败的教训写入你的日记中，然后在下一个循环换一个全新的思路重新尝试。

```
如果( Sharpe Ratio < 0.84 || 出错 ):
